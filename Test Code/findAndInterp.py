...
def get_properties_Horizons(asteroidsDf, time, loc: str = "500@10") -> pd.DataFrame:
    ...
    for name in asteroidsDf["Name"]:
        try:
            horizQ = Horizons(id=name, epochs=unitTime.jd,
                              location=loc, id_type="asteroid_name")
    ...
    eledf = pd.DataFrame(
        eleList, columns=["Name Horizons", "a", "e", "i", "H"])  # knownP
...
def find_asteroids(sector, cam, ccd, cut):
    ...
    badIds = []
    # ! ugly af, but takes all out of bounds values in X or Y, and gets them into the badIDs
    badIds += (np.where((xCoord < -1) | (xCoord >= fluxBounds) |
               (yCoord < -1) | (yCoord >= fluxBounds))[0].tolist())
    ...
    badIds = []
    # ! ugly af, but takes all out of bounds values in X or Y, and gets them into the badIDs
    badIds += (np.where((xCoord < 0) | (xCoord >= fluxBounds) |
               (yCoord < 0) | (yCoord >= fluxBounds))[0].tolist())
    # give extra space for interploations to work with
    ...
    if list(unqNames) == []:
        ...
        interpRes = pd.DataFrame(data=[[np.nan, f"There Were No Asteroids in {sector}_{cam}_{ccd}_{cut}", np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan]], columns=[
                                 "Num", "Name", "RA", "Dec", "Mv", "MJD", "FrameIDs", "X", "Y"])

        asteroidPropertiesDf = pd.DataFrame([[np.nan, f"There Were No Asteroids in {sector}_{cam}_{ccd}_{cut}", np.nan, np.nan, np.nan, f"There Were No Asteroids in {sector}_{cam}_{ccd}_{cut}", np.nan, np.nan, np.nan, np.nan]], columns=[
                                            "Num", "Name", "Mv(mean)", "Class", "Number of Points", "Name Horizons", "a", "e", "i", "H"])
        ...
        asteroidPropertiesDf = pd.DataFrame(propertiesList, columns=[
                                            "Num", "Name", "Mv(mean)", "Class", "Number of Points"])

        withEles = get_properties_Horizons(
            asteroidPropertiesDf, timeList[0], "500@10")
...
def interplolation_of_pos(posDf, sector):
    ...
    frameTimes = np.load(
        f"../OzData/sector{sector}_cam{cam}_ccd{ccd}_cut{cut}_of16_Times.npy")
    ...
    for name in unqNames:
        ...
        frameIDs = np.where((frameTimes >= minTime) &
                            (frameTimes <= maxTime))[0]

        # *gets IDs and times of frames all at once
        interpTimes = frameTimes[frameIDs]

        interpRAs = np.interp(
            x=interpTimes, xp=underSampledPos["MJD"], fp=underSampledPos["RA"])
        interpDecs = np.interp(
            x=interpTimes, xp=underSampledPos["MJD"], fp=underSampledPos["Dec"])
        interpDf = pd.DataFrame({"RA": interpRAs, "Dec": interpDecs, "MJD": interpTimes, "QueriedPoint": np.zeros_like(
            interpRAs), "FrameIDs": frameIDs.astype(int)})  # make into DF
        ...
        concatedDF.drop(
            concatedDF[concatedDF["QueriedPoint"] == 1].index, inplace=True)
        concatedDF.drop(columns=["QueriedPoint"], inplace=True)

        dfsList.append(concatedDF)

    if dfsList == []:
        interpRes = pd.DataFrame(data=[[np.nan, f"There Were No Asteroids in {sector}_{cam}_{ccd}_{cut}", np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan]], columns=[
                                 "Num", "Name", "RA", "Dec", "Mv", "MJD", "FrameIDs", "X", "Y", "Class"])
    else:
...
def plotExpectedPos(posDf: pd.DataFrame, timeList: npt.ArrayLike, targetPos: list, magLim: float = 20.0, scaleAlpha: bool = False, minLen: int = 0, saving=False, hsList=None) -> plt.Figure:
    ...
    if scaleAlpha:
        ...
        # doesn't like asking for None when it is a array
        if str(type(hsList)) == "<class 'NoneType'>":
            hs = {}
            for name in unqNames:
                try:
                    horizQ = Horizons(id=name, epochs=t_i.jd,
                                      location="500@10")
                    hs[name] = float(horizQ.elements()['H'])
                except Exception as e:
                    # some names aren't in jpl?
    ...
    if scaleAlpha:
        ax.text(-15, 1, s=scaleStr, fontsize=12)

    fig.tight_layout()

    if saving:
        fig.savefig(
            f"./Testing Figures/interpPos_{sector}_{cam}_{ccd}_{cut}.pdf")  # !CHECK
...
plotExpectedPos(resDf, frameTimes, setupQuery(sector, cam, ccd, cut),
                scaleAlpha=True, hsList=astrProps["H"].values, saving=True)
...
def arcstopx(x):
    """x from [-10.5, 10.5] want to map to [-0.5,0.5]"""
    return x/21


def pxtoarcs(x):
    """x from [-0.5,0.5] want to map to  [-10.5, 10.5]"""
    return 21*x
