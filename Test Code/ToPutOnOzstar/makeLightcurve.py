...
def sum_fluxes(frames, ys, xs, fluxBounds:list,reducedFluxes):
    ...
    NO LONGER Sums 3x3 box around all f,y,x, retuns list of fluxes in frame order
...
def lightcurve_from_name(name, interpDF,fluxBounds,reducedFluxes,basePath):
    ...
    targetWSC = fits.open(f"{basePath}wcs.fits")
    ...
    comFluxes = sum_fluxes(nameDf["FrameIDs"], nameDf["Y"], nameDf["X"], fluxBounds, reducedFluxes)
    nameDf["Flux"] = comFluxes 
