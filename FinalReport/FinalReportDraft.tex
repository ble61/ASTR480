...
A linear interpolation between the querried RA and Dec is preformed at the times of the TESS frames.
\texttt{TESSELLATE} saves out these time-frame pairs when doing its dataprocessing, allowing for the time coordinate of any position to be exactly at a frame time.
...
\subsection{Matching to \texttt{TESSELLATE} Detections}\label{SubSec:Match}
...
\subsection{Constructing Asteroid Ligthcurves}\label{SubSec:Lightcurves}
...
Many have no matches, and some have almost fully matched, this will be discussed more later. %ref
...
\subsection{Calculation of Rotation Periods}\label{SubSec:Periods}
...
\begin{figure}[!t]
  ...
  \caption[Periodogram example]{\textit{Upper Panel}:The Periodogram of the COM flux of an asteroid, with the power of each frequency displayed.
    ...
    \textit{Lower Panel}: The COM flux lightcurve and the returned model (dashed black curve) of the lightcurve from the periodogram analysis.
...
This is promising for those asteroids where no known frequency exists.

The stared peak in \autoref{Fig:PeriodEx} is by far the most prominent peak in the periodogram, as it is in log scale.
...
The periodogram itself must have a peak power of at least 0.1 to be distingused from the noise
...
\subsection{Quality Check Verification}
...
While some of the quality checks were chosen to agree with the past work of others, some were chosen to make sure the derived periods were in agreement with the known ones from the LCDB.
...
As seen in \autoref{Fig:PeriodEx} and expected from \citet{McNeill2023}, this line is what the periodogram should return due to double-peaked lightcurves.
...
On the whole, there are enough periods that pass the checks with incorrect periods that these extra erronious periods will not significantly change the fraction of reliable detections.
